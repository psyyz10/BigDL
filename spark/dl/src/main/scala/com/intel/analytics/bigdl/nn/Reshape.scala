  ...
  override def updateOutput(input: Tensor[T]): Tensor[T] = {
    ...
    if ((batchMode.nonEmpty && !batchMode.get) ||
      ...
      require(input.nElement() == nElement, s"element number must match Reshape size, " +
        s"input size is:${input.nElement()}, require size is $nElement")
      ...
      require(input.nElement() == nElement * input.size(1),
        s"element number must match Reshape size, input size is:${input.nElement()}" +
          s", require size is ${nElement * input.size(1)}, input is ${input.size().toList}")
